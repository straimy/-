...
public final class CosmeticCommands {
    ...
    private static int buy(ServerPlayer p,String id){
        ...
        p.sendSystemMessage(Component.literal("[GGO] Скин куплен навсегда: "+pretty(id)+" • -"+price+"◆").withStyle(ChatFormatting.LIGHT_PURPLE));
        return 1;
    }
    ...
    private static int equip(ServerPlayer p,String id){
        ...
        if("none".equals(id)){
            pr.equippedSkin="NONE";
            CosmeticSkinRuntime.applyAll(p,"NONE");
            GunnerArenaMod.RUNTIME.profiles().markDirty(p.getUUID());
            p.sendSystemMessage(Component.literal("[GGO] Скин снят.").withStyle(ChatFormatting.GRAY));
            return 1;
        }
        ...
        pr.equippedSkin=id;
        CosmeticSkinRuntime.applyAll(p,id);
        GunnerArenaMod.RUNTIME.profiles().markDirty(p.getUUID());
        p.sendSystemMessage(Component.literal("[GGO] Выбран скин: "+pretty(id)+" • применяется ко всем JEG-пушкам").withStyle(ChatFormatting.AQUA));
        return 1;
    }

    private static int list(ServerPlayer p){
        PlayerProfile pr=profile(p); if(pr==null)return 0;
        p.sendSystemMessage(Component.literal("[GGO] Скины: "+String.join(", ",pr.cosmetics)+" • выбран: "+pr.equippedSkin));
        return 1;
    }
