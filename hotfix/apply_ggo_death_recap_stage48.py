...
for name in ["GgoDeathRecapState.java","GgoKillFeedState.java","GgoDeathRecapAdapter.java","GgoDeathRecapBootstrap.java","GgoRespawnScreen.java"]:
...
feed=(JAVA/"GgoKillFeedState.java").read_text(encoding="utf-8")
...
for required in ["GgoDeathRecapNetwork","setClientConsumer","setKillFeedConsumer","GgoKillFeedState.add","Consumer<Object>"]:
...
for required in ["TTL_MS=8000L","ENTRIES.size()>5"]:
    if required not in feed: raise SystemExit(f"stage55 kill feed state missing: {required}")
if "new GgoRespawnScreen()" not in fence: raise SystemExit("stage48 death fence route missing")
print("Applied GGO Stage 48/55 client death presentation")
print(" - listener is installed before death so the packet cannot race the screen")
print(" - compact custom kill feed replaces vanilla death chat")
