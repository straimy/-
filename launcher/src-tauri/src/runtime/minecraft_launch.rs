...
fn build_launch(
    ...
    let main_class = forge.get("mainClass").or_else(|| vanilla.get("mainClass"))
        .and_then(Value::as_str).ok_or(LaunchError::MissingMainClass)?.to_string();
    let assets = vanilla.get("assets").and_then(Value::as_str)
        .or_else(|| vanilla.get("assetIndex").and_then(|v| v.get("id")).and_then(Value::as_str))
        .ok_or(LaunchError::MissingAssets)?.to_string();
    ...
    if !client_jar.exists() {
        return Err(LaunchError::MissingClientJar(client_jar.display().to_string()));
    }
    classpath.push(client_jar);
    let classpath_string = env::join_paths(&classpath)
        .map_err(|e| LaunchError::Runtime(format!("invalid classpath: {e}")))?
    ...
    let mut vars = vec![
        ...
        ("${game_directory}", install_dir.to_string_lossy().as_ref()),
        ("${assets_root}", install_dir.join("assets").to_string_lossy().as_ref()),
        ("${assets_index_name}", assets.as_str()),
        ("${version_type}", "release"),
        ("${natives_directory}", natives_dir.to_string_lossy().as_ref()),
        ("${launcher_name}", "GunGloryOnline"),
        ("${launcher_version}", env!("CARGO_PKG_VERSION")),
        ("${classpath}", classpath_string.as_str()),
    ];
    // Own the transient path strings before substitution.
    ...
    vars.retain(|(key, _)| !matches!(*key, "${game_directory}" | "${assets_root}" | "${natives_directory}"));
    vars.push(("${game_directory}", &game_dir_owned));
    vars.push(("${assets_root}", &assets_root_owned));
    vars.push(("${natives_directory}", &natives_owned));
    ...
    if game_args.is_empty() {
        if let Some(legacy) = vanilla.get("minecraftArguments").and_then(Value::as_str) {
            game_args.extend(split_legacy_args(legacy));
        }
        if let Some(legacy) = forge.get("minecraftArguments").and_then(Value::as_str) {
            game_args.extend(split_legacy_args(legacy));
        }
    }
    substitute_all(&mut game_args, &vars);
    game_args.extend(["--width".into(), options.width.clamp(640, 7680).to_string()]);
    game_args.extend(["--height".into(), options.height.clamp(480, 4320).to_string()]);
    ...
    Ok(BuiltLaunch {
        java_path,
        main_class,
        game_directory: install_dir.to_path_buf(),
...
fn collect_libraries(
    ...
    for library in libraries {
        ...
        let path = library.get("downloads").and_then(|d| d.get("artifact"))
        ...
        if !full.exists() {
            return Err(LaunchError::MissingLibrary(full.display().to_string()));
        }
...
fn rule_matches(rule: &Value) -> bool {
    ...
    if rule.get("features").is_some() { return false; }
    true
}

fn substitute_all(args: &mut [String], vars: &[(&str, &str)]) {
    for arg in args {
        for (key, value) in vars { *arg = arg.replace(key, value); }
    }
}

fn split_legacy_args(input: &str) -> Vec<String> {
    input.split_whitespace().map(ToOwned::to_owned).collect()
}

fn redact_token_args(mut args: Vec<String>) -> Vec<String> {
    for arg in &mut args {
        if arg.len() > 80 && !arg.starts_with('-') { *arg = "<redacted>".to_string(); }
    }
    args
}
...
mod tests {
    ...
    fn converts_maven_coordinate_to_library_path() {
        assert_eq!(
            maven_path("net.minecraftforge:forge:1.20.1-47.4.10:universal").unwrap(),
            PathBuf::from("net/minecraftforge/forge/1.20.1-47.4.10/forge-1.20.1-47.4.10-universal.jar")
        );
    }
