...
fn default_connect_server() -> bool {
    true
}
...
fn build_launch(
    ...
    let assets = vanilla
        ...
        .ok_or(LaunchError::MissingAssets)?
        .to_string();
    ...
    let client_jar = install_dir
        .join("versions")
        .join(inherited)
        .join(format!("{inherited}.jar"));
    if !client_jar.exists() {
        return Err(LaunchError::MissingClientJar(
    ...
    let classpath_separator_owned = if cfg!(windows) {
        ";".to_string()
    } else {
        ":".to_string()
    };
    ...
    let vars = vec![
        (
            "${auth_player_name}",
            session.minecraft_profile.name.as_str(),
        ),
        ("${auth_uuid}", session.minecraft_profile.id.as_str()),
        (
            "${auth_access_token}",
            session.minecraft_access_token.as_str(),
        ),
    ...
    let launch_mode = if options.launch_mode == "training" {
        "training"
    } else {
        "online"
    };
    ...
    game_args.extend([
        "--width".into(),
        options.width.clamp(640, 7680).to_string(),
        "--height".into(),
        options.height.clamp(480, 4320).to_string(),
    ]);
...
fn validate_custom_jvm_arg(arg: &str) -> Result<(), LaunchError> {
    ...
    let reserved = lower.starts_with("-xmx")
        ...
        || arg.contains("${");
    if reserved {
        return Err(LaunchError::UnsafeJvmArgument(arg.to_string()));
    }
    Ok(())
}
...
fn redact_token_args(mut args: Vec<String>) -> Vec<String> {
    for index in 0..args.len() {
        if index > 0 && args[index - 1] == "--accessToken" {
            args[index] = "<redacted>".to_string();
        }
    }
    args
}
...
mod tests {
    ...
    fn unresolved_placeholders_fail_closed() {
        let args = vec![
            ...
            "${library_directory}/mods".to_string(),
        ];
        assert!(matches!(
            ensure_no_placeholders(&args),
            Err(LaunchError::UnresolvedPlaceholder(_))
        ));
    }
    ...
    fn custom_jvm_args_cannot_override_launcher_critical_args() {
        ...
        assert!(matches!(
            validate_custom_jvm_arg("-Xmx8G"),
            Err(LaunchError::UnsafeJvmArgument(_))
        ));
        assert!(matches!(
            validate_custom_jvm_arg("--module-path=/tmp/x"),
            Err(LaunchError::UnsafeJvmArgument(_))
        ));
        assert!(matches!(
            validate_custom_jvm_arg("-Djava.library.path=/tmp/x"),
            Err(LaunchError::UnsafeJvmArgument(_))
        ));
    }
    ...
    fn launch_options_default_to_online_connection() {
        let options: LaunchOptions = serde_json::from_value(
            serde_json::json!({"ramMb":4096,"width":1280,"height":720,"fullscreen":false}),
        )
        .unwrap();
