...
impl BootstrapInfo {
    pub fn current() -> Self {
        // Stage109 is the unified GGO shell candidate: the launcher owns bootstrap visibility,
        // the Java/Forge engine becomes visible only after a first-party GGO readiness signal,
        // and vanilla navigation is fenced during official launcher sessions.
        ...
        let manifest_url = option_env!("GGO_MANIFEST_URL")
            ...
            .unwrap_or_else(|| {
                format!("{content_base_url}/manifests/beta-stage109-candidate.json")
            });

        Self {
            launcher_version: env!("CARGO_PKG_VERSION"),
            game_version: "v109-candidate",
            channel: "beta",
