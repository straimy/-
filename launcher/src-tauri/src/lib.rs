...
async fn launch_game(
    ...
    if fullscreen_after_ready {
        child_environment.push(("GGO_FULLSCREEN_AFTER_READY".to_string(), "1".to_string()));
    }
    ...
    let result = minecraft_process::launch_with_natives_environment(
        &root,
        custom_java.as_deref(),
        &session,
        &options,
